...
benchmark   200 RPS x 10s, 2/2/1 processes, seed 42
```
...
Catalogue scenarios define stable workload behavior. Profiles define scale and
environment choices such as target arrival rate, duration, execution-time
distribution, failure rate, and process counts.
...
### Attempts Failure Mode

Worker handlers cannot see the current retry count. `attempts` therefore behaves
like permanent failure and cannot model fail-first-N consistently across
multiple worker processes.

KQ should expose immutable delivery metadata to handlers, for example:

```go
retry, ok := kq.RetryCount(ctx)
if ok && retry < job.FailAttempts {
	return errSimulatedFailure
}
```

Until that API exists, the scenario must be reported as unsupported rather than
...
Single runs are useful for discovering large bottlenecks but do not establish
variance. The harness should support repeating the same resolved configuration,
grouping those run IDs, and reporting median results plus spread before small
performance differences drive implementation decisions.
