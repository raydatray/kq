...
Execution duration uses a deterministic lognormal model. `average` is required;
`p95`, `p99`, and `max` are optional. P99 determines the shape when present,
otherwise p95 does; supplying neither produces a constant duration. When both
percentiles are supplied, p95 must agree with the average-and-p99 fit. `max` is
a hard cap rather than a fitted lognormal parameter. Runs record the realized
average, p95, p99, and maximum.
...
execution times additionally report average, p95, p99, and maximum. Throughput is
...
- Execution workloads require an average and support optional p95, p99, and maximum durations.
