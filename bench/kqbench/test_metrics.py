"""Unit tests for measurement windows and warm-up exclusion."""
...
def scenario_events() -> list[dict]:
    return [
        ...
        # Warm-up task executed after a 5s cold start.
        ...
        # Measured tasks on a warm share group.
...
class SummarizeTest(unittest.TestCase):
    def test_windows_exclude_warmup(self) -> None:
        ...
        # Enqueue window spans measured starts/finishes only.
        self.assertAlmostEqual(windows["enqueue"], 2000.0)
        # Active window spans measured handler activity only.
        self.assertAlmostEqual(windows["active"], 1150.0)
        # Drain spans workload end (8.0) to the final terminal (8.15).
        self.assertAlmostEqual(windows["drain"], 150.0)
        # Startup keeps the warm-up execution: ready (0.0) to first start (5.0).
