...
def resolve_run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    # Base workload from profile, with scenario behavior mapped in.
    arrival = dict(profile.get("arrival", {}))
    if "arrival" in adhoc:
        arrival = deep_merge(arrival, dict(adhoc["arrival"]))

    if "execution_time_ms" in behavior and behavior["execution_time_ms"]:
        execution_time = dict(behavior["execution_time_ms"])
    else:
        execution_time = dict(profile.get("execution_time_ms", {}))
    if "execution_time_ms" in adhoc:
        execution_time = deep_merge(execution_time, dict(adhoc["execution_time_ms"]))

    profile_failures = dict(profile.get("failures", {}))
    failures: dict[str, Any] = {
        "rate": behavior.get("failure_rate", profile_failures.get("rate", 0)),
