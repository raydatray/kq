...
def resolve_run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    topology: dict[str, Any] = {
        ...
        "producer_concurrency": topology_profile.get("producer_concurrency", 1),
...
def validate_resolved_config(resolved: dict[str, Any]) -> None:
    ...
    if "average" not in execution:
        raise ValueError("execution_time_ms.average is required")
    for key in ("average", "p95", "p99", "max"):
        if key in execution and float(execution[key]) < 0:
            raise ValueError(f"execution_time_ms.{key} cannot be negative")
    average = float(execution["average"])
    optional = [float(execution[key]) for key in ("p95", "p99", "max") if key in execution]
    if average == 0 and any(value != 0 for value in optional):
        raise ValueError("zero execution average cannot have percentiles or maximum")
    if "p95" in execution and float(execution["p95"]) < average:
        raise ValueError("execution_time_ms.p95 cannot be below average")
    if "p99" in execution and float(execution["p99"]) < average:
        raise ValueError("execution_time_ms.p99 cannot be below average")
    if "p95" in execution and "p99" in execution and float(execution["p99"]) < float(execution["p95"]):
        raise ValueError("execution_time_ms.p99 cannot be below p95")
    if "max" in execution:
        maximum = float(execution["max"])
        for key in ("average", "p95", "p99"):
            if key in execution and maximum < float(execution[key]):
                raise ValueError(f"execution_time_ms.max cannot be below {key}")
    if failures["mode"] not in ("none", "permanent", "until_time"):
    ...
    for key in ("producers", "producer_concurrency", "workers", "worker_concurrency", "movers"):
