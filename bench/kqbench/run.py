...
def run_harness(config: dict[str, Any], output: Path) -> int:
    ...
    # Artifacts are written after cleanup so duration_ms spans Kafka start
    # through cleanup and event collection observes every trailing line.
...
def run_warmup_phase(
    ...
    """Drain the unmeasured warm-up range to prove share-group usability.

    Completing warm-up tasks demonstrates workers receive records; only then
    do measured producers start. An empty warm-up set skips the phase so
    cold-start behavior stays measurable on demand.
    """
    if not warmup_ids:
        return
