...
from .config import resolve_run, resolve_sweep_runs, validate_resolved_config
...
class SweepResolutionTest(unittest.TestCase):
    ...
    @patch("kqbench.config.get_environment", return_value={})
    @patch("kqbench.config.get_revision", return_value="abcdef123456")
    def test_rejects_invalid_execution_distribution(self, _revision: object, _environment: object) -> None:
        args = parse_args([
            "run",
            "--scenario", "ready-success-v1",
            "--profile", "ci-smoke",
        ...
        resolved = resolve_run(args)
        execution = resolved["workload"]["execution_time_ms"]

        del execution["average"]
        with self.assertRaisesRegex(ValueError, "average is required"):
            validate_resolved_config(resolved)

        execution["average"] = 5
        execution["max"] = 4
        with self.assertRaisesRegex(ValueError, "max cannot be below average"):
            validate_resolved_config(resolved)
