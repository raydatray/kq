...
// DurationDistribution describes target execution-time shape.
//
// Average, P95, and P99 do not uniquely define a distribution. This sampler
// uses deterministic quantile selection:
//
//   - 95% of draws fall below P95, shaped by a power curve calibrated to
//     Average.
//   - 4% of draws interpolate linearly from P95 to P99.
//   - 1% of draws equal P99.
//
// The power curve preserves the P95/P99 quantile boundaries while letting
// Average control skew within the head: a low average concentrates mass
// near zero (high power), while an average near P95/2 approaches uniform
// (power near 1). Requested averages outside the achievable range clamp to
// power [0, maxPower] and the run records the actual generated quantiles.
...
// distributionJSON is the millisecond wire format.
...
// MarshalJSON encodes durations as milliseconds.
...
// UnmarshalJSON decodes millisecond numbers into durations.
...
// Sample draws one deterministic duration from rng.
...
// power calibrates head skew so the expected value approaches Average.
//
// Head contribution is 0.95*P95/(power+1); tail contribution is fixed by the
// linear P95-P99 band plus the P99 point mass. Solving for power yields the
// skew that matches Average when achievable.
