...
func TestModeForSelection(t *testing.T) {
	...
	if got := (FailureConfig{Rate: 0, Mode: FailureModePermanent}).ModeFor(rng); got != FailureModeNone {
		t.Fatalf("rate 0 mode = %q, want none", got)
	}
	if got := (FailureConfig{Rate: 1, Mode: FailureModePermanent}).ModeFor(rng); got != FailureModePermanent {
		t.Fatalf("rate 1 mode = %q, want permanent", got)
	}
	if got := (FailureConfig{Rate: 1, Mode: FailureModeNone}).ModeFor(rng); got != FailureModeNone {
		t.Fatalf("none mode = %q, want none", got)
	}

	config := FailureConfig{Rate: 0.5, Mode: FailureModePermanent}
...
func TestConfigValidation(t *testing.T) {
	...
	bad := good
	bad.Failures.Mode = FailureMode("bogus")
