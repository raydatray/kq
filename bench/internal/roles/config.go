// Package roles implements kq-load process roles.
package roles
...
// Config is the resolved run configuration consumed by Go role processes.
type Config struct {
	KQ           KQConfig       `json:"kq"`
...
// KQConfig carries broker and retry settings for the run.
...
// TopologyConfig carries process counts.
...
// LoadConfig reads resolved configuration from path.
...
// TotalTasks returns the requested workload size.
func (c Config) TotalTasks() uint64 {
	return uint64(c.Workload.Arrival.TargetRPS) * uint64(c.Workload.Arrival.DurationSeconds)
}

// WorkloadIDs returns the deterministic ID assignment for this producer.
...
// ToKQConfig builds the KQ client configuration with fixed retry delay.
