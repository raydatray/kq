...
// RunWorker executes bench tasks with simulated latency and failure behavior.
...
// applyFailureBehavior materializes the deterministic failure mode.
//
// Supported modes are none, permanent, and until_time. The attempts mode
// remains skipped until KQ exposes retry count to handlers; it currently
// fails like permanent so retry paths are still exercised.
func applyFailureBehavior(job workload.Job, now time.Time) error {
	...
	case "until_time":
		...
		return nil
	case "attempts":
		return errors.New("bench: attempts failure (retry count unavailable)")
	default:
